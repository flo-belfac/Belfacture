...
export default function Factures() {
  ...
  const [lignes, setLignes] = useState([{ description: '', quantite: '', prix_unitaire: '', taux_tva: 21 }])
  ...
  async function creerFacture() {
    ...
    const { data: facture, error } = await supabase.from('factures').insert([{ user_id: user.id, client_id: clientId, numero, total_htva: htva, total_tva: tva, total_tvac: tvac, statut: 'brouillon' }]).select().single()
  ...
  function genererPDF(factureData: any, e: any) {
    const doc = new jsPDF()
    doc.setFontSize(18); doc.setTextColor(219, 110, 68); doc.setFont('helvetica', 'bold')
    ...
    doc.setFontSize(22); doc.setTextColor(40, 30, 20); doc.setFont('helvetica', 'bold')
    ...
    doc.setDrawColor(219, 110, 68); doc.setLineWidth(0.8); doc.line(20, 60, 190, 60)
    doc.setFontSize(10); doc.setTextColor(130, 100, 80); doc.text('Facturé à :', 20, 72)
    doc.setFontSize(13); doc.setTextColor(40, 30, 20); doc.setFont('helvetica', 'bold')
    doc.text(factureData.client?.nom || '', 20, 81)
    if (factureData.client?.numero_tva) { doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(100, 80, 60); doc.text('TVA : ' + factureData.client.numero_tva, 20, 88) }
    doc.setFillColor(219, 110, 68); doc.rect(20, 98, 170, 9, 'F')
    ...
    ;(factureData.lignes || []).forEach((l: any, i: number) => {
      ...
      doc.setTextColor(40, 30, 20); doc.setFontSize(9)
    ...
    doc.setDrawColor(219, 110, 68); doc.setLineWidth(0.4); doc.line(120, y + 4, 190, y + 4)
    doc.setFontSize(10); doc.setTextColor(100, 80, 60); doc.text('Total HTVA :', 122, y + 13)
    doc.setTextColor(40, 30, 20); doc.text(factureData.total_htva.toFixed(2) + ' €', 188, y + 13, { align: 'right' })
    doc.setTextColor(100, 80, 60); doc.text('TVA :', 122, y + 21)
    doc.setTextColor(40, 30, 20); doc.text(factureData.total_tva.toFixed(2) + ' €', 188, y + 21, { align: 'right' })
    doc.setFillColor(219, 110, 68); doc.rect(118, y + 25, 74, 11, 'F')
    doc.setTextColor(255, 255, 255); doc.setFont('helvetica', 'bold')
    doc.text('Total TVAC :', 122, y + 32.5); doc.text(factureData.total_tvac.toFixed(2) + ' €', 188, y + 32.5, { align: 'right' })
    if (e.iban) { doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(130, 100, 80); doc.text('Paiement par virement :', 20, y + 50); doc.setTextColor(40, 30, 20); doc.text('IBAN : ' + e.iban + (e.bic ? '   BIC : ' + e.bic : ''), 20, y + 57); doc.text('Communication : ' + factureData.numero, 20, y + 64) }
    doc.setFontSize(8); doc.setTextColor(180, 160, 140); doc.text('Facture générée via BelFacture — belfacture.vercel.app', 105, 285, { align: 'center' })
  ...
  async function telechargerPDF() {
    ...
    genererPDF(derniereFacture, e)
    setPdfEnCours(null)
  }

  async function telechargerPDFExistant(facture: any) {
    setPdfEnCours(facture.id)
    const { data: l } = await supabase.from('lignes_facture').select('*').eq('facture_id', facture.id)
    const e = await getParams()
    genererPDF({ ...facture, lignes: l || [] }, e)
    setPdfEnCours(null)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(120% 90% at 30% 0%, #241a12 0%, #15110d 55%)', fontFamily: "'Figtree', sans-serif" }}>
      <style>{`
        ...
        .pdf-btn{background:rgba(219,110,68,0.1);border:1px solid rgba(219,110,68,0.25);border-radius:8px;color:#db6e44;padding:5px 10px;font-size:12px;font-weight:600;cursor:pointer;font-family:'Figtree',sans-serif;}
        .pay-btn{background:rgba(126,211,168,0.1);border:1px solid rgba(126,211,168,0.25);border-radius:8px;color:#7ed3a8;padding:5px 10px;font-size:12px;font-weight:600;cursor:pointer;font-family:'Figtree',sans-serif;margin-left:4px;}
      ...
      <div style={{ maxWidth: 920, margin: '0 auto', padding: '48px 24px' }}>
        <h1 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 52, fontWeight: 400, color: '#f2ebdc', margin: '0 0 40px', letterSpacing: '-0.01em' }}>Nouvelle facture</h1>
        ...
        <div style={card}>
          ...
          {lignes.map((ligne, i) => (
            <div key={i} className="ligne-row">
              ...
              {lignes.length > 1 ? <button onClick={() => setLignes(lignes.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', color: 'rgba(240,138,99,0.5)', cursor: 'pointer', fontSize: 18, padding: 0 }}>×</button> : <span />}
            </div>
          ))}
          <button onClick={() => setLignes([...lignes, { description: '', quantite: '', prix_unitaire: '', taux_tva: 21 }])} style={{ color: '#db6e44', background: 'rgba(219,110,68,0.08)', border: '1px solid rgba(219,110,68,0.2)', borderRadius: 9, cursor: 'pointer', fontSize: 13, fontWeight: 600, padding: '8px 16px', marginTop: 8, fontFamily: "'Figtree',sans-serif" }}>
            + Ajouter une ligne
          </button>
        </div>
        ...
        <div style={{ ...card, textAlign: 'right' as const }}>
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 6 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: 'rgba(242,235,220,0.45)', fontSize: 14 }}>Total HTVA</span><span style={{ color: 'rgba(242,235,220,0.7)', fontWeight: 600 }}>{htva.toFixed(2)} €</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: 'rgba(242,235,220,0.45)', fontSize: 14 }}>TVA</span><span style={{ color: 'rgba(242,235,220,0.7)', fontWeight: 600 }}>{tva.toFixed(2)} €</span></div>
        ...
        <button onClick={creerFacture} style={{ width: '100%', padding: 16, background: '#db6e44', border: 'none', borderRadius: 13, color: '#15110d', fontSize: 16, fontWeight: 700, cursor: 'pointer', marginBottom: 12, boxShadow: '0 8px 24px rgba(219,110,68,.3)', fontFamily: "'Figtree',sans-serif" }}>Créer la facture</button>

        {derniereFacture && (
          <button onClick={telechargerPDF} disabled={pdfEnCours === derniereFacture?.id} style={{ width: '100%', padding: 16, background: 'rgba(126,211,168,0.15)', border: '1px solid rgba(126,211,168,0.3)', borderRadius: 13, color: '#7ed3a8', fontSize: 16, fontWeight: 700, cursor: 'pointer', marginBottom: 16, fontFamily: "'Figtree',sans-serif" }}>
        ...
        <div style={card}>
          ...
          {mesFactures.length === 0 ? <p style={{ color: 'rgba(242,235,220,.3)', textAlign: 'center', padding: 24 }}>Aucune facture</p>
            : mesFactures.map(f => (
              <div key={f.id} className="facture-row">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  <button className="pdf-btn" onClick={() => telechargerPDFExistant(f)} disabled={pdfEnCours === f.id}>{pdfEnCours === f.id ? '⏳' : '📄 PDF'}</button>
