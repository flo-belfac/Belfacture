...
export default function Parametres() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(120% 90% at 30% 0%, #111820, #0d1117 55%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(120% 90% at 30% 0%, #111820 0%, #0d1117 55%)', fontFamily: "'Figtree', sans-serif" }}>
      <style>{`
        .par-input:focus{border-color:#3b82f6 !important;}
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '48px 24px' }}>
        <h1 style={{ fontFamily: "'Instrument Serif', serif", color: '#e8f0fe', fontSize: 40, fontWeight: 400, margin: '0 0 8px', letterSpacing: '-0.01em' }}>Paramètres</h1>
        <p style={{ color: 'rgba(232,240,254,0.5)', margin: '0 0 32px', fontSize: 14 }}>Infos entreprise, Peppol et sécurité de votre compte.</p>

        <div style={card}>
          <h2 style={{ color: '#3b82f6', fontSize: 13, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase' as const, margin: '0 0 20px' }}>Votre entreprise</h2>
        ...
        <div style={card}>
          <h2 style={{ color: '#3b82f6', fontSize: 13, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase' as const, margin: '0 0 10px' }}>Peppol & Paiement</h2>
          <p style={{ color: 'rgba(232,240,254,0.4)', fontSize: 12, marginBottom: 18, lineHeight: 1.6 }}>
            Identifiant Peppol = <code style={{ background: 'rgba(59,130,246,0.15)', color: '#3b82f6', padding: '2px 6px', borderRadius: 4 }}>0208:</code> + numéro d&apos;entreprise sans points.
          </p>
          <div style={{ display: 'grid', gap: 14 }}>
        ...
        <div style={card}>
          <h2 style={{ color: '#3b82f6', fontSize: 13, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase' as const, margin: '0 0 20px' }}>Adresse légale</h2>
          <div style={{ display: 'grid', gap: 14 }}>
        ...
        <button onClick={handleSave} disabled={saving} style={{ width: '100%', padding: 16, background: '#3b82f6', border: 'none', borderRadius: 13, color: '#0d1117', fontSize: 16, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 24px rgba(59,130,246,.35)', marginBottom: 16, opacity: saving ? 0.7 : 1, fontFamily: "'Figtree',sans-serif" }}>
          {saving ? '⏳ Sauvegarde...' : '💾 Sauvegarder les paramètres'}
        </button>

        {message && <p style={{ textAlign: 'center', marginBottom: 20, fontSize: 15, color: message.includes('Erreur') ? '#f87171' : '#4ade80', fontWeight: 600 }}>{message}</p>}

        <div style={card}>
          <h2 style={{ color: '#3b82f6', fontSize: 13, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase' as const, margin: '0 0 22px' }}>🔐 Sécurité</h2>
          <div style={{ marginBottom: 24 }}>
            <p style={{ color: '#e8f0fe', fontWeight: 600, fontSize: 15, margin: '0 0 14px' }}>Changer le mot de passe</p>
            ...
            <button className="sec-btn" onClick={handleChangePassword} disabled={changingPass} style={{ marginTop: 12, padding: '12px 22px', background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: 11, color: '#3b82f6', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: "'Figtree',sans-serif" }}>
              {changingPass ? '⏳...' : 'Changer le mot de passe'}
            </button>
          </div>
          <div style={{ borderTop: '1px solid rgba(59,130,246,0.1)', paddingTop: 20 }}>
            <p style={{ color: '#e8f0fe', fontWeight: 600, fontSize: 15, margin: '0 0 14px' }}>Changer l&apos;adresse email</p>
            ...
            <p style={{ color: 'rgba(232,240,254,0.35)', fontSize: 12, margin: '8px 0 12px', lineHeight: 1.5 }}>Un lien de confirmation sera envoyé à la nouvelle adresse.</p>
            <button className="sec-btn" onClick={handleChangeEmail} style={{ padding: '12px 22px', background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: 11, color: '#3b82f6', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: "'Figtree',sans-serif" }}>
              Changer l&apos;email
            </button>
          </div>
          {passMessage && <p style={{ marginTop: 14, fontSize: 14, fontWeight: 600, color: passMessage.includes('Erreur') || passMessage.includes('pas') || passMessage.includes('invalide') ? '#f87171' : '#4ade80' }}>{passMessage}</p>}
        </div>
      </div>
    </div>
  )
